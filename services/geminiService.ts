...
Avoid generic AI suggestions. Always anchor on measurable economic value.`;
  ...
  const response = await ai.models.generateContent({
    ...
    contents: `Analyze the following business/sector and generate a full Predictive Intelligence Strategy Report: ${details}`,
    config: {
      systemInstruction,
...
Avoid generic AI buzzwords. Prioritize business impact and compounding advantage.`;
  ...
  const response = await ai.models.generateContent({
    model: "gemini-3-pro-preview",
    contents: `Analyze the following business/sector for a Predictive Data Flywheel: ${details}`,
    config: {
      systemInstruction,
      responseMimeType: "application/json",
      responseSchema: {
...
Avoid generic AI buzzwords. Focus on architectural blueprints that turn data into continuously improving predictions.`;
  ...
  const response = await ai.models.generateContent({
    model: "gemini-3-pro-preview",
    contents: `Design an AI Platform Architecture for the following business context: ${details}`,
    config: {
      systemInstruction,
      responseMimeType: "application/json",
      responseSchema: {
...
Always prioritize economic clarity. Always show formulas. Always quantify impact.`;
  ...
  const response = await ai.models.generateContent({
    model: "gemini-3-pro-preview",
    contents: `Generate a comprehensive ROI report for the following business or sector: ${details}`,
    config: {
      systemInstruction,
      responseMimeType: "application/json",
      responseSchema: {
...
8. Investment Recommendation: Title and Justification.`;
  ...
  const response = await ai.models.generateContent({
    model: "gemini-3-pro-preview",
    contents: `Analyze the following business for a comprehensive ICVF valuation: ${details}`,
    config: {
      systemInstruction,
      responseMimeType: "application/json",
      responseSchema: {
